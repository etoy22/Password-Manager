...
def Display():
        ...
        if(test['Info']!=[]):
            ...
            for i in range(0,len(test['Info'])):
                Button1=Button(root,text="Send",command=lambda i=i:sending(test['Info'][i][2],test['Info'][i][1]))
                Button1.place(x=1000,y=yValue)
                Button2=Button(root,text="Delete",command=lambda i=i:sending(test['Info'][i][2],test['Info'][i][1]))
