...
def send_account(rec_username,sevice_id):
    ...
    Tag:2 resent an account to the same person
...
def get_send_pass(rec_user,service_id):
    ...
    '''
    result = send(ApplicationStates.GET_SENDING_PASSWORD.value,rec_user,service_id)
    return result
...
def delete_rec(sending_username,service_id):
    result = send(ApplicationStates.DELETE_REC.value,sending_username,service_id)
    return result
    
def sent_password():
    result = send(ApplicationStates.GET_SENDING_ACCOUNT.value)
    return result
