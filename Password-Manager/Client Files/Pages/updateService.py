...
def checkService():
    ...
    check=client.check_service(int(entry_1.get()))
    print(check)
    if(check['Password']==None):
        ...
        messagebox.showinfo("Password",check['Password'])
def changeService():
...
def deleteService():
    ...
    print(client.get_services())
    check=client.deleteService(int(entry_1.get()))
    print(check)
    if(check==0):
        messagebox.showerror(None,"Not a valid service ID for this account try choosing a different service ID")
    elif(check==1):
...
def Display():
        ...
        label4 = tk.Label(text = "User ID: ",bg = "white", width = 15, font = ("arial", 10,"bold"))   
        ...
        delete=tk.Button(text="Delete",command=deleteService)
        delete.place(x=80,y=330)
        check=tk.Button(text="Check",command=checkService)
        check.place(x=150,y=330)
