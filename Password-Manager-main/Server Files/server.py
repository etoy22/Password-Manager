            ...
            # Delete account
                ...
                if adr in account_tracker: 
                    ...
                    for i in range(len(data)):
                        ...
                        temp = get_severice_data(data[i][0],data[i][1])
                ...
                if adr in account_tracker:
                    user_id = account_tracker[adr]
                    data = get_out_going(user_id)
                    for i in range(len(data)):
                        data[i] = list(data[i])
                        data[i].append(get_username(data[i][0]))
                        temp = get_severice_data(user_id,data[i][1])
                        data[i].append(temp[0])
                        data[i].append(temp[1])
                    accessed = {
                        "Tag":1,
                ...
                if adr in account_tracker:
                    ...
                    service_id = msg['SEC']
                    user_id = account_tracker[adr]
                    delete_sending(user_id,rec_user_id,service_id)
                    accessed = {
                        "Tag":1
                    }
                    send_client(accessed)
                ...
                if adr in account_tracker:
                    ...
                    service_id = msg['SEC']
                    user_id = account_tracker[adr]
                    delete_sending(send_user_id,user_id,service_id)
                    accessed = {
                        "Tag":1
                    }
                    send_client(accessed)
